...
fun CommonTextField(
    ...
    OutlinedTextField(
        ...
        leadingIcon = if (leadingIcon != null) {
            {
                Icon(
                    ...
                    tint = if (isError) ErrorNeonRose else TextFieldIcon
                )
            }
        } else null,
        trailingIcon = {
        ...
        colors = OutlinedTextFieldDefaults.colors(
            ...
            focusedBorderColor = if (isError) ErrorNeonRose else TextFieldBorder,
            unfocusedBorderColor = if (isError) ErrorNeonRose else TextFieldBorder,
            focusedTextColor = TextFieldText,
            unfocusedTextColor = TextFieldText,
            focusedLabelColor = if (isError) ErrorNeonRose else TextFieldBorder,
            unfocusedLabelColor = if (isError) ErrorNeonRose else TextFieldHint,
            ...
            cursorColor = if (isError) ErrorNeonRose else TextFieldBorder
        )
    )
}
...
fun UsernameTextField(
    ...
    CommonTextField(
        ...
        modifier = modifier.height(AppDimensions.TextFieldHeight)
    )
}
...
@Preview(showBackground = true, backgroundColor = 0xFF020613, showSystemUi = true)
@Composable
fun TextFieldPreview() {
    ...
    MaterialTheme {
        Surface(modifier = Modifier.fillMaxSize(), color = Color(0xFF020613)) {
            Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
                UsernameTextField(value = username, onValueChange = { username = it })
                Spacer(modifier = Modifier.height(10.dp))
                EmailTextField(value = email, onValueChange = { email = it }, isError = true)
                Spacer(modifier = Modifier.height(10.dp))
